...
struct rte_hash *flow_tables[12];
...
//	memset(&port_conf, 0, sizeof(struct rte_eth_conf));
	...
	for (q = 0; q < rx_rings; q++) {
		...
		snprintf(mempool_name, sizeof(mempool_name), "mbuf_pool_%u_%u", port, q);
		
		// 为每个RX队列创建一个独立的mempool
		struct rte_mempool *mbuf_pool_per_queue = rte_pktmbuf_pool_create(mempool_name, 
																		NUM_MBUFS, 
																		MBUF_CACHE_SIZE, 
																		0, 
																		RTE_MBUF_DEFAULT_BUF_SIZE, 
																		rte_eth_dev_socket_id(port));
		if (mbuf_pool_per_queue == NULL) {
			rte_exit(EXIT_FAILURE, "Cannot create mbuf pool for queue %u\n", q);
		}

		// 使用为当前队列创建的mempool来设置RX队列
		retval = rte_eth_rx_queue_setup(port, q, nb_rxd,
										rte_eth_dev_socket_id(port), NULL, mbuf_pool_per_queue);
		if (retval < 0) {
			rte_exit(EXIT_FAILURE, "Cannot setup rx queue %u\n", q);
		}
	}
...
/*				
				uint32_t rss_hash = bufs[buf]->hash.rss;
...
int main(int argc, char *argv[])
{
	...
	const int BUCKETS_COUNT = rte_lcore_count() - 1; // 假设您有8个buckets
	for (int i = 0; i < BUCKETS_COUNT; i++) {
		buckets[i] = (flow *)rte_malloc("bucket", Flow_num * sizeof(flow), RTE_CACHE_LINE_SIZE);
		if (buckets[i] == NULL) {
			// 处理内存分配失败的情况
			rte_exit(EXIT_FAILURE, "Failed to allocate memory for bucket %d\n", i);
		}
	}


	const int TABLE_COUNT = rte_lcore_count() - 1; // 假设您有8个表
