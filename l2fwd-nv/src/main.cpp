...
//申请内存
	const int size = 1024;
//	DataPacket a[size];
	DataPacket* a = new DataPacket[size];
	DataPacket *d_a;
    for (int i = 0; i < size; ++i) {
        a[i].id = i;
    }

    cudaMalloc((void**)&d_a, size * sizeof(DataPacket));
    cudaMemcpy(d_a, a, size * sizeof(DataPacket), cudaMemcpyHostToDevice);
	...
	while (RTE_GPU_VOLATILE(force_quit) == 0 && RTE_GPU_VOLATILE(p_v->pipeline_force_quit) == 0) {
		...
		if (p_v->workload_type == GPU_PK_WORKLOAD || p_v->workload_type == GPU_GRAPHS_WORKLOAD) {
			...
			workload_launch_gpu_processing(d_a,&(p_v->comm_list[bindex]), conf_pktime_ns,
								MAC_CUDA_BLOCKS, 1024, p_v->c_stream);

/*
			DataPacket* h_a = (DataPacket*)malloc(2 * sizeof(DataPacket));
			cudaMemcpy(h_a, d_a, 2 * sizeof(DataPacket), cudaMemcpyDeviceToHost);
			printf("data %d\n",h_a[1].id);
*/
			POP_RANGE;
		} else
