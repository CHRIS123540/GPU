...
lcore_main(__rte_unused void *arg)
{
    ...
    if (!is_time_initialized) {
        clock_gettime(CLOCK_REALTIME, &global_time_start);
        is_time_initialized = 1; // 标记时间已被记录
		printf("0\n");
	}
    ...
    for (int i = 0; i < 100; i++) { // 打印前 10 个字节或整个 payload
	...
	if(total >= 1000){
		clock_gettime(CLOCK_REALTIME, &global_time_end);
		printf("1\n");
	}
                }
...
/*此处预计放置打印的内容*/
    ...
    //    write(pipe_fd, data, 2); // 写入数字“1”和换行符
		...
		//printf("coo\n");
    double time_diff = (global_time_end.tv_sec - global_time_start.tv_sec) +
                       (global_time_end.tv_nsec - global_time_start.tv_nsec) / 1e9;
